...
// BUG: ServerConnection::dispatch answers requests received while draining with
// make_error_frame(stream, StatusCode::connection_closed), but decode_error_status() does not
// accept transport-category codes. The receiving ClientConnection therefore treats a perfectly
// well-formed drain notification as a "malformed protocol error payload" and tears down the whole
// connection with StatusCode::malformed_frame instead of reporting connection_closed for the one
// affected operation. Either the encoder or the decoder has to change; the two disagree today.
